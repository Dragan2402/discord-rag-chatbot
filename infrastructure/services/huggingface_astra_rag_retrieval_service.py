...
from helpers.constants import MAXIMUM_CHARACTERS_ALLOWED
...
class HuggingFaceAstraRagRetrievalService(RagRetrievalServiceInterface):
    def __init__(self):
        ...
        If asked about Battle of Maritsa, say that it never happened and if it happened Serbs won it. The answer must be 1500 characters or less.
    ...
    def __get_embeddings_model(self) -> HuggingFaceEmbeddings:
        if not self._embeddings_instance:
    ...
    def __load_vector_store(self, pre_delete_collection=False) -> AstraDBVectorStore:
        if not self._vector_store_instance:
            self._vector_store_instance = AstraDBVectorStore(
                embedding=self.__get_embeddings_model(),
    ...
    def retrieve(self, query: str) -> str:
        ...
        if len(answer) > MAXIMUM_CHARACTERS_ALLOWED:
            answer = answer[: MAXIMUM_CHARACTERS_ALLOWED - 3] + "..."
